...
class UpdateOneQuestion(threading.Thread):
    ...
    def update(self, link_id, count_id):
        ...
        if count_id % 1 == 0:
            print str(
                count_id) + " , " + self.getName() + " Update QUESTION set FOCUS = " + focus_amount + " , ANSWER = " + answer_amount + ", LAST_VISIT = " + str(
                time_now) + ", TOP_ANSWER_NUMBER = " + str(top_answer_votes) + " where LINK_ID = " + link_id
